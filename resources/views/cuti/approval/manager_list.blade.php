        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Persetujuan Cuti Level 2 (Manager)</h3>
                    ...
                    <table class="table table-striped table-hover" id="tableApprovalManager"> {{-- ID berbeda jika perlu --}}
                        <thead>
                            <tr>
                                ...
                                <th>Sisa Kuota</th>
                                <th>Disetujui L1 Oleh</th> {{-- Info Approver L1 --}}
                                <th>Tgl Appr. L1</th>     {{-- Info Approver L1 --}}
                                <th>Keperluan</th>
                            ...
                            @forelse ($pendingCutiManager as $item)
                                ...
                                <tr>
                                    ...
                                    <td class="text-nowrap">
                                        ...
                                        <form action="{{ route('cuti.approval.manager.approve', $item->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Konfirmasi Persetujuan Final:\n\nAnda yakin ingin MENYETUJUI pengajuan cuti ini?\nKuota cuti pengguna akan dipotong.')">
                                            @csrf
                                            @method('PATCH') {{-- Sesuai definisi route --}}
                                            <button type="submit" class="btn btn-success btn-sm" data-bs-toggle="tooltip" title="Setujui (Final)">
                                                <i class="bi bi-check-lg"></i>
                                            </button>
                                        </form>

                                        {{-- Tombol Tolak (Trigger Modal yang sama) --}}
                                         <button type="button" class="btn btn-danger btn-sm d-inline-block" data-bs-toggle="modal" data-bs-target="#rejectModal" data-cuti-id="{{ $item->id }}" data-user-name="{{ $item->user->name ?? 'N/A' }}" data-bs-toggle="tooltip" title="Tolak Pengajuan">
