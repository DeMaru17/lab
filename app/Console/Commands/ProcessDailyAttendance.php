...
// use App\Notifications\AttendanceCorrectionReminder;
...
class ProcessDailyAttendance extends Command
{
    ...
    protected $description = 'Process daily attendance records for the last N days to determine final status.';
    ...
    public function handle()
    {
        ...
        foreach ($dateRange as $currentProcessDate) {
            ...
            foreach ($usersToProcess as $user) {
                 if ($user->tanggal_mulai_bekerja->gt($currentProcessDate)) continue;
                 ...
                 try {
                    ...
                    $attendance = Attendance::with('shift') // Ambil data attendance jika ada
                        ->where('user_id', $user->id)
                        ->where('attendance_date', $currentProcessDate->toDateString())
                        ->first();
                    $shiftIdToSave = $attendance->shift_id ?? null; // Pertahankan shift jika record sudah ada
                    ...
                    if (!$finalStatus && $isHolidayOrWeekend) {
                        ...
                        if ($hasAttendanceRecord && $approvedOvertime) {
                             ...
                             // shiftIdToSave sudah dipreserve dari $attendance
                             ...
                             $finalStatus = 'Libur'; // Tetap Libur jika tidak ada absen
                             $finalNotes = 'Lembur disetujui tapi tidak ada data absensi.';
                             $this->info("    -> Status: Libur (Lembur tanpa Absensi)");
                             $isNormalAttendance = false; // Bukan absensi normal
                             $shiftIdToSave = null;
                        }
                         // Jika $hasAttendanceRecord && !$approvedOvertime, maka $isNormalAttendance tetap true
                         // dan akan diproses di bawah seolah hari biasa.
                    }

                    // --- 4. Proses Absensi Normal / Alpha (Jika tidak Cuti/Dinas/Libur/Lembur) ---
                    if ($isNormalAttendance) {
                        if (!$attendance) {
                            $finalStatus = 'Alpha'; $finalNotes = 'Tidak ada data check-in/check-out.';
                            $this->warn("    -> Status: Alpha (No record)");
                            $shiftIdToSave = null;
                        } elseif (!$attendance->clock_in_time || !$attendance->clock_out_time) {
                            $finalStatus = 'Alpha'; // Set Alpha jika tidak lengkap
                            ...
                            $this->warn("    -> Status: Alpha (Data Tidak Lengkap)");
                            // --- TODO: Trigger Reminder Koreksi ---
                            // shiftIdToSave sudah dipreserve
                        } else {
                            // Data Lengkap -> Hitung Hadir/Terlambat/Pulang Cepat
                            $shift = $attendance->shift;
                            if (!$shift) { throw new \Exception("Shift tidak ditemukan (ID: {$attendance->shift_id}) untuk absensi ID {$attendance->id}."); }
                             $this->info("    -> Shift: {$shift->name} ({$shift->start_time->format('H:i')} - {$shift->end_time->format('H:i')})");

                             // ... (Logika perhitungan $isLate, $isEarly, $lateMinutes, $earlyLeaveMinutes) ...
                    ...
                    Attendance::updateOrCreate(
                        [
                            'user_id' => $user->id,
                            'attendance_date' => $currentProcessDate->toDateString(),
                        ],
                        [
                            'attendance_status' => $finalStatus,
                            'notes' => $finalNotes,
                            'shift_id' => $shiftIdToSave, // Shift null jika Cuti/Dinas/Libur/Alpha(baru)
                            // Jaga data clock in/out jika record sudah ada & statusnya bukan yg meniadakan jam kerja
                            'clock_in_time' => ($isNormalAttendance && $attendance) ? $attendance->clock_in_time : null,
                            'clock_out_time' => ($isNormalAttendance && $attendance) ? $attendance->clock_out_time : null,
                            // Jaga data lainnya juga
